...
/**
 * Ported from php-api's Services/StrategyService.php (itself ported from Services/StrategyService.cs).
 * Strategy CRUD + activate — every method takes organisationId first and filters by it, same
 * cross-org-isolation discipline as PortfolioService. Activating a Strategy is the one place
 * IsActive is ever written — flips every other Strategy in the same org to false in the same
 * transaction. `IsActive` is cast (bool) on every read — PDO_MYSQL returns TINYINT(1) as a plain
 * PHP int, never a real bool (mariadb-api/CLAUDE.md §4.8).
 */
final class StrategyService
{
    ...
    public function create(string $organisationId, array $request): array
    {
        ...
        $strategyId = Uuid::v4();
        $stmt = $this->db->prepare('INSERT INTO "Strategies" ("Id", "OrganisationId", "Name", "IsActive", "SortOrder", "DateCreated") VALUES (:id, :orgId, :name, 0, :sortOrder, now())');
    ...
    public function activate(string $organisationId, string $strategyId): ?array
    {
        ...
        try {
            $this->db->prepare('UPDATE "Strategies" SET "IsActive" = 0 WHERE "OrganisationId" = :orgId AND "Id" != :id AND "IsActive" = 1')
                ->execute(['orgId' => $organisationId, 'id' => $strategyId]);
            $this->db->prepare('UPDATE "Strategies" SET "IsActive" = 1 WHERE "Id" = :id')->execute(['id' => $strategyId]);
            $this->db->commit();
        } catch (\Throwable $e) {
