...
final class PortalHomeService
{
    ...
    /** Ported from php-api/src/Services/PortalHomeService.php (itself ported from
     * PortalHomeService.VoteQaEntryNpsAsync) — no dialect divergence. End-user thumbs-up/down
     * voting — "up" is +1, anything else is -1, no floor/ceiling, no per-user vote tracking (a
     * simple tally, not a persistent per-user ledger, per this feature's own deliberately minimal
     * spec). Gated the same way every other read/write here is: the Portal must be published AND
     * the caller must actually have an access grant for it. */
     ...
     * reopen a saved Draft with its previously-entered answers bound back into the form.
