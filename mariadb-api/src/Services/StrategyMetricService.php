...
/**
 * Ported from php-api's Services/StrategyMetricService.php (itself ported from
 * Services/StrategyMetricService.cs). Metric CRUD (enforcing the exactly-one-parent rule) plus
 * append-only StrategyMetricEntry recording/history. RecordedAt is always server-set via SQL now(),
 * never a PHP-constructed timestamp bound as a parameter, so mariadb-api/CLAUDE.md §4.6's
 * ISO-8601-reformat gotcha doesn't apply anywhere in this file.
 */
final class StrategyMetricService
{
    ...
    /** ProjectMember-readable variant — resolves the project's own org first, then re-uses the same
     * ownership-checked getHistory. */
