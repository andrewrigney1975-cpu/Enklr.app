...
final class AiAssistantService
{
    ...
    /** Ported from php-api's own listAvailableFormsTool. The set of Forms is org-wide and changes
     * over time — deliberately NOT baked into the system prompt, queried fresh on every call instead.
     * Delegates entirely to FormSubmissionService::getAuthorableForms, which already re-derives the
     * caller's own Author-gate satisfaction server-side. */
    ...
    /** Ported from php-api's own getFormFieldsTool. Re-resolves formId to its currently-published
     * version EVERY call, then describes every field's id/type/required-ness/options AND the exact
     * value shape submit_form expects for that field. */
    ...
    /** Ported from php-api's own fieldValueShapeDescription — plain-English instruction for exactly
     * what JSON shape submit_form's "answers" value must be for this field. */
    ...
    /** Ported from php-api's own submitFormTool. Validates the model-constructed "answers" object
     * against the form's CURRENT published version, then reuses FormSubmissionService::create
     * (Draft + answers) followed immediately by submit() — deliberately NOT reimplementing the
     * Author-gate check here, since submit()'s own independent re-derivation is the actual security
     * boundary. If the workflow raised a Task, surfaces it as an ordinary task_created action. */
