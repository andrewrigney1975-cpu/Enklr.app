...
/**
 * Ported from php-api/src/Services/PortalService.php (itself ported from Services/PortalService.cs).
 * Backs Org-Admin authoring of Organisational Portals — gated by OrgAdminMiddleware only (see
 * routes.php), same cross-org isolation stance as PortfolioService: every id a client supplies is
 * independently re-validated against the caller's own organisation id before anything is touched.
 * No dialect divergence from the Postgres tier anywhere in this file.
 */
final class PortalService
{
    ...
    public function create(string $organisationId, string $callerUserId, array $request): array
    {
        ...
        try {
            ...
            // php-api/src/Services/PortalService.php's identical block for the full rationale.
    ...
    /** Ported from php-api/src/Services/PortalService.php (itself ported from
     * PortalService.ReorderTopicAsync) — no dialect divergence. Moves a Topic up/down among all of
     * a Portal's own topics — see applyReorder()'s own doc comment for the renumber-then-swap
     * self-healing this relies on. A Topic's own QaEntries are untouched by this — they stay tagged
     * to this topic via PortalTopicId regardless of the topic's Order, so they visually move WITH
     * their topic for free once the frontend groups entries under topics in topic-Order sequence. */
    ...
    /** Ported from php-api/src/Services/PortalService.php (itself ported from
     * PortalService.ReorderQaEntryAsync) — no dialect divergence. Moves an entry up/down among its
     * own siblings only — same PortalTopicId (including the ungrouped/null bucket) — matching how
     * the admin Q&A tab groups entries under topic headers; an entry never reorders across topics
     * this way (moving it to a different topic is what updateQaEntry's own portalTopicId field is
     * for). */
     ...
     * can't parameterize identifiers. Transaction-wrapped DML (not DDL), so unlike Migrator.php's
     * own MariaDB gotcha (§4.1), a normal beginTransaction/commit/rollBack works fine here. */
