...
/**
 * Ported from php-api's Services/StrategyPillarService.php (itself ported from
 * Services/StrategyPillarService.cs). Pillar + Enabler CRUD combined in one class. Every method
 * re-validates ownership up the chain (Pillar -> Strategy -> Organisation, Enabler -> Pillar ->
 * Strategy -> Organisation) before touching anything. No boolean columns here, so no (bool)-cast
 * concern (mariadb-api/CLAUDE.md §4.8) applies to this file.
 */
final class StrategyPillarService
{
    ...
    public function getActivePillarTreeForProject(string $projectId): ?array
    {
        ...
        $stmt = $this->db->prepare('SELECT "Id" FROM "Strategies" WHERE "OrganisationId" = :orgId AND "IsActive" = 1');
