...
/**
 * Ported from php-api's Services/VendorService.php (itself ported from Services/VendorService.cs).
 * OrgAdmin-facing CRUD for Vendor records, plus per-Vendor API key generate/revoke — folded into
 * this one service rather than split out the way OrganisationApiKeyService is its own class, same
 * rationale as the other two tiers. Every id is re-validated against the caller's own
 * organisationId before anything is touched — no enumeration oracle.
 *
 * MariaDB-specific divergence from the Postgres tier (mariadb-api/CLAUDE.md §4.8): IsActive is cast
 * (bool) on read, since PDO_MYSQL returns TINYINT(1) as a plain int, never a real bool. `now()` and
 * literal `true`/`false` in SQL text (not bound as parameters) work unchanged under ANSI_QUOTES.
 */
