...
/**
 * Ported from php-api/src/Services/FormAnswerValidator.php (itself ported from
 * Services/FormAnswerValidator.cs) — see that file's own doc comment for the full rationale
 * (validates an AI-Assistant-constructed AnswersJson object against a Form's own FieldsJson schema
 * before FormSubmissionService::create/submit ever sees it; AnswersJson has no server-side
 * validation for the human fill-out path today, since its rendered widgets already enforce
 * required/type/option constraints before an answer can even be typed in — an LLM has no such
 * guardrail, so this is a genuinely NEW validation path scoped to the AI Assistant's submit_form
 * tool only). Pure PHP-array/JSON logic, no SQL — no dialect divergence from the Postgres tier.
 *
