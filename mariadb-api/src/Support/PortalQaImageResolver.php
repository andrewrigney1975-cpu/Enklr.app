...
/**
 * Ported from php-api/src/Support/PortalQaImageResolver.php (itself ported from
 * Services/PortalQaImageResolver.cs). Resolves a PortalQaEntry's header image ONCE, at Create/Update
 * time (called from PortalService::createQaEntry/updateQaEntry) — never re-resolved on read.
 * Searches Pexels using keywords extracted from the entry's own Question+Answer text ("keyword
 * density" — the most frequent non-stopword terms, Question terms weighted double). Best-effort
 * throughout: a missing API key, a Pexels outage, a timeout, or zero search results all fall back to
 * a persisted random colour rather than ever blocking or failing the entry's own save. Plain cURL,
 * same zero-HTTP-client-dependency style as this tier's own AiAssistantService.php call to
 * Anthropic. No dialect divergence from the Postgres tier anywhere in this file (pure PHP/HTTP, no
 * SQL involved).
 */
