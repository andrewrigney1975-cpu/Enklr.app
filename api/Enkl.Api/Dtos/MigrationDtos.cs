...
public record ImportColumnDto(string Id, string Name, bool Done, string? Color, int Order, int Cap = -1, bool ColorBackground = true, bool IsBlocked = false);
public record ImportReleaseDto(string Id, string Name, string Status, string? OwnerId, string? StartDate, string? EndDate, string? DateCreated, string? DateLastModified, string? Color);
