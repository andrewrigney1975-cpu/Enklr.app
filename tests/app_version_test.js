...
(async () => {
  ...
  const versionMatch = html.match(/var APP_VERSION = '([^']+)'/);
  log('APP_VERSION constant is defined', !!versionMatch, versionMatch && versionMatch[1]);
  const version = versionMatch ? versionMatch[1] : '';
  const formatRe = /^\d+\.\d{2,}\.\d{8}\.\d{4}$/;
  log('format is major.minor.yyyymmdd.hhmm', formatRe.test(version), version);

  const parts = version.split('.');
  log('major version starts at 1', parts[0] === '1', parts[0]);
  ...
  const versionMatchAfterImport = html.match(/var APP_VERSION = '([^']+)'/);
  log('the running app\u2019s own APP_VERSION constant is untouched by the import', versionMatchAfterImport[1] === version);
