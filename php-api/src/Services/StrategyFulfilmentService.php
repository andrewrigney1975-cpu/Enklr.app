...
/**
 * Ported from Services/StrategyFulfilmentService.cs. ProjectPillarFulfilment upsert (called from
 * Portfolio Planner's per-project Strategy modal) and the fulfilment-matrix read that feeds all three
 * radar views (per-project, portfolio-aggregate, multi-project overlay).
 */
final class StrategyFulfilmentService
{
     ...
     * against the caller's org — a legitimate own-org project paired with another org's pillar id
     * must fail closed (root CLAUDE.md §4's "second FK gets its own re-validation" rule). Value is
     * clamped 0-100. */
     ...
     * belong to the caller's org before touching anything, so a foreign-org id is silently dropped
     * rather than surfaced as an error. An empty/omitted list means "every project in the org." */
