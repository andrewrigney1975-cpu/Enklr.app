...
/**
 * Ported from Services/WhiteboardService.cs — see that file's own comments for the "why" behind each
 * piece. Org-wide collaborative whiteboard sessions, no ProjectMember concept applies. Every lookup
 * re-derives the session from the caller's own OrganisationId (never trusted from the client) and,
 * for host-only actions, the caller's own UserId against the session's stored HostUserId. A wrong
 * join code and a right code for a different org's session return the identical "not found" (no
 * enumeration oracle).
 */
final class WhiteboardService
{
    ...
    public function createSession(string $organisationId, string $callerUserId, ?string $title): array
    {
        ...
        try {
            $this->db->prepare(
                ...
                'code' => $joinCode, 'title' => $trimmedTitle, 'saved' => false, 'created' => $dateCreated,
            ]);
            $this->db->prepare(
    ...
    {
        $stmt = $this->db->prepare(
            'UPDATE "WhiteboardParticipants" p SET "LeftAt" = :left
             FROM "WhiteboardSessions" s
             WHERE p."SessionId" = s."Id" AND p."SessionId" = :sid AND p."UserId" = :uid AND p."LeftAt" IS NULL AND s."OrganisationId" = :orgId'
        );
        $stmt->execute(['left' => gmdate('Y-m-d\TH:i:s\Z'), 'sid' => $sessionId, 'uid' => $callerUserId, 'orgId' => $organisationId]);
        if ($stmt->rowCount() === 0) {
            return null;
        }
    ...
    public function saveSession(string $organisationId, string $callerUserId, string $sessionId): bool
    {
        ...
        $stmt->execute(['saved' => true, 'savedAt' => gmdate('Y-m-d\TH:i:s\Z'), 'sid' => $sessionId, 'orgId' => $organisationId, 'uid' => $callerUserId]);
    ...
    public function closeSession(string $organisationId, string $callerUserId, string $sessionId): ?array
    {
        ...
        )->execute(['closed' => gmdate('Y-m-d\TH:i:s\Z'), 'sid' => $sessionId]);
     ...
     *
     * @return string[]|null Every currently-present participant's user id (including the caller;
     *   the frontend itself ignores a cursor event whose userId is its own, which is the more
     *   precise filter for a purely-cosmetic signal like this one), or null if the caller isn't a
     *   current participant of an open session in their own org.
     */
