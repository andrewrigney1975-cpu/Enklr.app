...
final class FormSubmissionService
{
    ...
    /** Ported from FormSubmissionService.cs's GetAuthorableFormsAsync — every currently-published
     * Form in the org whose workflow's Author node the caller's own gates satisfy. This is the AI
     * Assistant's list_available_forms tool's entire implementation (AiAssistantService.php), reusing
     * this class's own gate-matching primitives directly rather than exposing them for a second
     * implementation elsewhere. A misconfigured Form (no Start->Author edge) is silently excluded
     * rather than erroring the whole list — matches submit()'s own "not accepting submissions yet"
     * rejection for the same case.
    ...
    /** Re-resolves a Form to its own currently-published row (org-scoped, so a caller can never probe
     * another organisation's Form by guessing its id) — the shared "is this still the current
     * version" re-check both get_form_fields and submit_form need, always re-run fresh rather than
     * trusting a formId a model may have seen several conversation turns ago. */
