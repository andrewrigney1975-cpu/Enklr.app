...
/** Ported from Controllers/WhiteboardController.cs. Org-wide collaborative whiteboard —
 * RequireAuthMiddleware only (no ProjectMember/OrgAdmin), since any org user can start or join a
 * session, mirroring ChatController's own org-wide, no-extra-policy shape. */
final class WhiteboardController extends BaseController
{
    ...
    public function cursorMove(Request $request, Response $response, array $args): Response
    {
        $body = $this->body($request);
        $otherParticipantUserIds = $this->service()->getOtherParticipantUserIdsForCursor($this->callerOrgId($request), $this->callerUserId($request), $args['id']);
        if ($otherParticipantUserIds === null) {
            return $this->notFound($response);
        }

        try {
            (new Broadcaster(Database::connection()))->broadcastWhiteboardCursorMoved(
                $otherParticipantUserIds, $args['id'], $this->callerUserId($request), $this->callerDisplayName($request) ?? 'Someone',
                (float) ($body['x'] ?? 0), (float) ($body['y'] ?? 0)
            );
        } catch (\Throwable) {
